#!/bin/bash
# ============================================================
# U-Claw - Install to Linux (从 U 盘安装到电脑)
...
if [ "$ARCH" = "x86_64" ]; then
    ...
    NODE_PLATFORM="node-linux-x64"
elif [ "$ARCH" = "aarch64" ]; then
    echo -e "  ${GREEN}Linux ARM64 ✓${NC}"
    NODE_PLATFORM="node-linux-arm64"
else
...
case $USE_NODE in
    ...
    download)
        ...
        PLATFORM_NAME="linux-$( [ "$ARCH" = "x86_64" ] && echo "x64" || echo "arm64" )"
...
cat > "$INSTALL_TARGET/start.sh" << 'STARTEOF'
#!/bin/bash
DIR="$(cd "$(dirname "$0")" && pwd)"
ARCH=$(uname -m)
NODE_PLATFORM="node-linux-$( [ "$ARCH" = "x86_64" ] && echo "x64" || echo "arm64" )"
...
DESKTOP_FILE="$HOME/.local/share/applications/u-claw.desktop"
mkdir -p "$HOME/.local/share/applications"
cat > "$DESKTOP_FILE" << DESKEOF
[Desktop Entry]
Name=U-Claw
Comment=Portable AI Agent
Exec=bash $INSTALL_TARGET/start.sh
Terminal=true
Type=Application
Categories=Utility;
DESKEOF
...
echo ""
echo -e "  ${BOLD}启动方式:${NC}"
echo -e "    终端运行: ${CYAN}bash ~/.uclaw/start.sh${NC}"
echo -e "    或在应用菜单搜索 ${CYAN}U-Claw${NC}"
echo ""
echo -e "  ${BOLD}首次使用:${NC}"
